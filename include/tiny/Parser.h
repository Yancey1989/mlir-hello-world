...
#ifndef MLIR_TUTORIAL_TINY_PARSER_H
#define MLIR_TUTORIAL_TINY_PARSER_H
...
private:
  ...
  std::unique_ptr<ExprAST> str2IntLiteralExpr(tiny::Location& loc, std::string& str)  {
  ...
  std::unique_ptr<ExprAST> parseStrExpr() {
    ...
    for(auto& c: lexer.getStr()) {
      auto result =
        std::make_unique<NumberExprAST>(loc, int(c));
...
#endif // MLIR_TUTORIAL_TINY_PARSER_H
